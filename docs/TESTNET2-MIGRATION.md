...
**What this is:** how to deploy the updated bots (`kbbot`, `viktor`, `chess-bot`, `unicity-l3`) after the migration to `@unicitylabs/sphere-sdk@0.10.3` on **testnet2**. Source: PR #12, branch `migrate/sphere-sdk-0.9.1-testnet2` (branch name predates the final version; the pinned SDK is **0.10.3**).
...
4. **Lockfile is already regenerated and committed** for 0.10.3 — Docker `--frozen-lockfile` builds will succeed. Do **not** hand-edit `pnpm-lock.yaml`.
